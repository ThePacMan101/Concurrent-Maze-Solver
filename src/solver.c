...
vec2_t move_direction(vec2_t pos, direction_t dir) {
    vec2_t new_pos = pos;
    switch(dir) {
        case NORTH: new_pos.y--; break;
        case SOUTH: new_pos.y++; break;
        case EAST:  new_pos.x++; break;
        case WEST:  new_pos.x--; break;
        default: break;
    }
    return new_pos;
}
...
static vec2_t calculate_mutex_grid_dimensions(vec2_t maze_dimensions) {
    ...
    int_t grid_x = (maze_dimensions.x + REGION_SIZE - 1) / REGION_SIZE;  // Ceiling division
...
static void alloc_exploration_map(exploration_map_t *exp_map, maze_t maze) {
    ...
    exp_map->mutex_grid.grid_dimensions = calculate_mutex_grid_dimensions(maze.dimensions);
    exp_map->mutex_grid.region_size = REGION_SIZE;
    
    size_t num_mutexes = exp_map->mutex_grid.grid_dimensions.x * exp_map->mutex_grid.grid_dimensions.y;
    exp_map->mutex_grid.mutexes = (pthread_mutex_t*) malloc(num_mutexes * sizeof(pthread_mutex_t));
    
    if (!exp_map->mutex_grid.mutexes) {
        PERROR("Couldn't allocate mutex grid with size: %d x %d", 
               exp_map->mutex_grid.grid_dimensions.x, exp_map->mutex_grid.grid_dimensions.y);
    }
...
static void init_bifurcation_buffer(bifurcation_buffer_t *buffer, int_t capacity) {
...
void init_solver_state(solver_state_t *state, maze_t maze, uint8_t num_workers, bool enable_iterative_visualization) {
    state->maze = maze;
    state->num_workers = num_workers;
    state->goal = (vec2_t){maze.dimensions.x - 1, maze.dimensions.y - 1}; // hardcoded solution
    ...
    state->worker_positions = (worker_position_t*) calloc(num_workers, sizeof(worker_position_t));
    ...
    pthread_mutex_init(&state->viz_mutex, NULL);
}
...
void cleanup_solver_state(solver_state_t *state) {
    ...
    pthread_mutex_destroy(&state->viz_mutex);
}
...
static direction_t get_available_directions(solver_state_t *state, vec2_t pos) {
    ...
    if (available & NORTH && !explored_at(state->explored, pos.x, pos.y - 1)) result |= NORTH;
    if (available & SOUTH && !explored_at(state->explored, pos.x, pos.y + 1)) result |= SOUTH;
    if (available & EAST && !explored_at(state->explored, pos.x + 1, pos.y)) result |= EAST;
    if (available & WEST && !explored_at(state->explored, pos.x - 1, pos.y)) result |= WEST;
    ...
    while (true) {
        ...
        if (should_terminate) {
            ...
            if (held_region_mutex != -1) {
                pthread_mutex_unlock(&state->explored.mutex_grid.mutexes[held_region_mutex]);
            }
            break;
        }
        ...
        if (!is_actively_exploring) {
            ...
            while (state->bifurcations.count == 0 && !state->shutdown && !state->solution_found) {
            ...
            if (state->bifurcations.count > 0) {
                ...
                state->bifurcations.head = (state->bifurcations.head + 1) % state->bifurcations.capacity;
            ...
            if (work_acquired) {
                ...
                int_t new_region = get_mutex_index(state->explored, current_position.x, current_position.y);
        ...
        int_t region_mutex_idx = get_mutex_index(state->explored, current_position.x, current_position.y);
        ...
        bool cell_already_explored = explored_at(state->explored, current_position.x, current_position.y);
        if (!cell_already_explored) {
            explored_at(state->explored, current_position.x, current_position.y) = true;
        ...
        bool reached_goal = (current_position.x == state->goal.x && current_position.y == state->goal.y);
        if (reached_goal) {
        ...
        direction_t unexplored_directions = get_available_directions(state, current_position);
        ...
        if (num_paths == 0) {
            ...
            current_position = move_direction(current_position, unexplored_directions);
                ...
                if (unexplored_directions & dir) {
                    if (chosen_direction == 0) {
                        ...
                        if (state->bifurcations.count < state->bifurcations.capacity) {
                            state->bifurcations.data[state->bifurcations.tail].position = branch_position;
                            state->bifurcations.data[state->bifurcations.tail].came_from = opposite_direction(dir);
                            state->bifurcations.tail = (state->bifurcations.tail + 1) % state->bifurcations.capacity;
                            state->bifurcations.count++;
                            
                            pthread_cond_signal(&state->bifurcations.work_available);  // Wake one idle worker
                        }
                    }
                }
            }

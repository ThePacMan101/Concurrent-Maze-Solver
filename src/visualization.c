...
void mark_worker_active_at_position(solver_state_t *state, uint8_t worker_id,
                                    vec2_t position) {
...
void update_worker_position(solver_state_t *state, uint8_t worker_id,
                            vec2_t position) {
  pthread_mutex_lock(&state->viz_mutex);
  state->worker_positions[worker_id].position = position;
  pthread_mutex_unlock(&state->viz_mutex);
  ...
  for (int y = 0; y < maze.dimensions.y; y++) {
    for (int x = 0; x < maze.dimensions.x; x++) {
      ...
      if (x > 0) {
        ...
        if (explored)
          wprintf(L"\033[36m"); // ANSI cyan
        print_spchar(middle_ch);
        if (explored)
          wprintf(L"\033[0m"); // ANSI reset
      }
...
void print_maze_with_solution(solver_state_t *state) {
  ...
  bool *is_on_path = (bool *)calloc(
      maze.true_dimensions.x * maze.true_dimensions.y, sizeof(bool));
  ...
  while (!(current.x == 0 && current.y == 0)) {
    ...
    if (came_from == 0)
      break; // Shouldn't happen if solution exists
...
void print_maze_animated(solver_state_t *state) {
  ...
  // Worker background color codes (different bright backgrounds for each
  // worker)
    ...
    for (int x = 0; x < maze.dimensions.x; x++) {
      ...
      if (x > 0) {
        ...
        if (worker_here >= 0 || worker_left >= 0) {
          ...
          // Color if explored
          bool explored =
              explored_at(*exp, x, y) || explored_at(*exp, x - 1, y);
          if (explored)
      ...
      if (x == 0 && y == 0) {
        wprintf(L"\033[42m\033[1m\033[30m"); // Green background + bold + black
                                             // text (start)
      } else if (x == state->goal.x && y == state->goal.y) {
        wprintf(L"\033[41m\033[1m\033[97m"); // Red background + bold + white
                                             // text (goal)
      } else if (worker_here >= 0) {
        wprintf(
            L"%ls\033[1m\033[30m",
            worker_bg_colors[worker_here % num_colors]); // Worker background +
                                                         // bold + black text
      } else if (is_explored) {
        wprintf(L"\033[2m\033[36m"); // Dim cyan for explored
      }
